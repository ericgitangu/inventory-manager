...
const handler = NextAuth({
  ...
  callbacks: {
    ...
    async signIn({ user, account, profile, email, credentials }) {
      console.log("SignIn Callback:", { user, account, profile, email, credentials }); // Logging the signIn callback data
      return true;
    },
    ...
    async jwt({ token, user, account, profile, isNewUser }) {
      console.log("JWT Callback:", { token, user, account, profile, isNewUser }); // Logging the JWT callback data
      if (account) {
